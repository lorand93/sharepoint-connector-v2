...
export class ContentFetchingStep implements IPipelineStep {
  ...
  async execute(context: ProcessingContext): Promise<ProcessingContext> {
    ...
    try {
      ...
      this.logger.log(
        `[${context.correlationId}] Downloading file content - Drive: ${driveId}, Item: ${itemId}`,
      );

      // Download file content from SharePoint
      const contentBuffer = await this.sharepointApiService.downloadFileContent(
        driveId,
        itemId,
      );
      ...
      const maxFileSizeBytes =
        this.configService.get<number>('pipeline.maxFileSizeBytes') ||
        209715200; // 200MB

      if (fileSizeBytes > maxFileSizeBytes) {
        throw new Error(
          `File size ${fileSizeBytes} bytes exceeds maximum limit of ${maxFileSizeBytes} bytes`,
        );
      }
      ...
      this.logger.log(
        `[${context.correlationId}] Content fetching completed for file: ${context.fileName} (${Math.round(fileSizeBytes / 1024 / 1024)}MB)`,
      );
      ...
      this.logger.error(
        `[${context.correlationId}] Content fetching failed: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  private extractDriveId(context: ProcessingContext): string | null {
    ...
    this.logger.warn(
      `[${context.correlationId}] Drive ID not found in metadata. Available keys: ${Object.keys(metadata).join(', ')}`,
    );
    return null;
  }
  ...
  private validateMimeType(mimeType: string, correlationId: string): void {
    const allowedMimeTypes =
      this.configService.get<string[]>('sharepoint.allowedMimeTypes') || [];
    ...
    this.logger.log(
      `[${correlationId}] MIME type validation passed: ${mimeType}`,
    );
  }
